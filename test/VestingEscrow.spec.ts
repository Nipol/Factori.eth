...
describe('VestingEscrow', () => {
  ...
  beforeEach(async () => {
    ...
    const VestingEscrowTemplate = await ethers.getContractFactory(
      'contracts/utils/VestingEscrow.sol:VestingEscrow',
      wallet,
    );

    VestingEscrow = await VestingEscrowTemplate.deploy();
  });

  describe('#fund()', () => {
    beforeEach(async () => {
      await VestingEscrow.initialize(StandardToken.address);
    });

    it('should be success fund', async () => {
      await StandardToken.approve(VestingEscrow.address, initialToken);
      await expect(VestingEscrow['fund(uint256)'](initialToken))
        .to.emit(VestingEscrow, 'Funded')
        .withArgs(initialToken);
      expect(await StandardToken.balanceOf(VestingEscrow.address)).to.equal(initialToken);
    });
    ...
    it('should be revert fund with sig, not enough balance', async () => {
      ...
      await StandardToken.transfer(to, initialToken);
  ...
  describe('#lock()', () => {
    ...
    it('should be revert with already registered', async () => {
      ...
      expect(await VestingEscrow.allocatedSupply()).to.equal('1');
  ...
  describe('#claim()', () => {
    ...
    it('should be success claim with one second', async () => {
      ...
      expect(await VestingEscrow.allocatedSupply()).to.equal(BigNumber.from('99000000000000000000'));
    });
    ...
    it('should be success claim with two second', async () => {
      ...
      expect(await VestingEscrow.allocatedSupply()).to.equal(BigNumber.from('98000000000000000000'));
    });
    ...
    it('should be success claim with two calls', async () => {
      ...
      await expect(VestingEscrow['claim(address)'](to))
        .to.emit(VestingEscrow, 'Claimed')
        .withArgs(to, BigNumber.from('1000000000000000000'));
      expect(await VestingEscrow.allocatedSupply()).to.equal(BigNumber.from('97000000000000000000'));
    });
    ...
    it('should be success with end time', async () => {
      ...
      expect(await VestingEscrow.allocatedSupply()).to.equal(BigNumber.from('0'));
    });
  });
  ...
  describe('#decreaseLockedOf()', () => {
    ...
    it('should be success desc with claim', async () => {
      ...
      expect(await VestingEscrow.allocatedSupply()).to.equal(BigNumber.from('49000000000000000000'));
      ...
      expect(await VestingEscrow.allocatedSupply()).to.equal(BigNumber.from('48500000000000000000'));
    });
