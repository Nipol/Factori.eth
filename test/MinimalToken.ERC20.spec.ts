...
describe('MinimalToken/ERC20', () => {
  ...
  beforeEach(async () => {
    ...
    const templateStandardToken = await StandardTokenTemplate.deploy();
    ...
    const tkey = keccak256(defaultAbiCoder.encode(['address', 'uint256'], [templateStandardToken.address, nonce]));
    ...
    expect(await Factory.addTemplate(templateStandardToken.address))
      .to.emit(Factory, 'NewTemplate')
      .withArgs(tkey, templateStandardToken.address, deployableBeaconAddr);
    ...
    const calculatedAddress = await Factory.connect(Dummy)['compute(bool,bytes32,bytes)'](false, tkey, data);
    ...
    StandardToken = StandardTokenTemplate.attach(calculatedAddress);
  });
